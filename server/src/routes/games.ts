...
import { GameModel, NewGame } from '../models/game';
import { GameScoreModel, NewGameScore } from '../models/gameScore';

interface ParamsWithId {
  id: string;
}

interface ParamsWithGameIdAndPlayerId {
  gameId: string;
  playerId: string;
}
...
export default async function gameRoutes(fastify: FastifyInstance) {
  ...
  fastify.get('/', async (_request: FastifyRequest, reply: FastifyReply) => {
    try {
      const games = await GameModel.getGamesWithPlayers();
  ...
  fastify.get('/:id', async (request: FastifyRequest<{ Params: ParamsWithId }>, reply: FastifyReply) => {
    try {
      ...
      const gameWithScores = await GameScoreModel.getGameWithScores(gameId);
  ...
  fastify.put('/:id', async (request: FastifyRequest<{ Params: ParamsWithId, Body: NewGame }>, reply: FastifyReply) => {
    try {
      const { id } = request.params;
      const game = request.body;
      
      const updatedGame = await GameModel.update(parseInt(id), game);
  ...
  fastify.delete('/:id', async (request: FastifyRequest<{ Params: ParamsWithId }>, reply: FastifyReply) => {
    try {
      const { id } = request.params;
      const deleted = await GameModel.delete(parseInt(id));
  ...
  fastify.get('/:id/scores', async (request: FastifyRequest<{ Params: ParamsWithId }>, reply: FastifyReply) => {
    try {
      const { id } = request.params;
      const scores = await GameScoreModel.getByGameId(parseInt(id));
  ...
  fastify.post('/:gameId/players/:playerId', async (
    request: FastifyRequest<{ Params: ParamsWithGameIdAndPlayerId }>,
    reply: FastifyReply
  ) => {
    try {
      ...
      for (const frame of frames) {
        await GameScoreModel.create(frame);
      }
  ...
  // Update a score
  fastify.put('/:gameId/scores', async (
    request: FastifyRequest<{ Params: { gameId: string }, Body: NewGameScore }>,
    reply: FastifyReply
  ) => {
    try {
      ...
      const existingScores = await GameScoreModel.getByGameAndPlayer(
        scoreUpdate.game_id,
        scoreUpdate.player_id
      );
      ...
      // Update the score
      const updatedScore = await GameScoreModel.update(existingScore.id, scoreUpdate);
