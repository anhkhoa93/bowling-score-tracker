...
export class GameScoreModel {
   ...
   */
  static async create(score: NewGameScore): Promise<GameScore> {
    const query = `
   ...
   */
  static async getByGameId(gameId: number): Promise<GameScore[]> {
    const query = `
   ...
   */
  static async getByGameAndPlayer(gameId: number, playerId: number): Promise<GameScore[]> {
    const query = `
      SELECT * FROM game_scores 
   ...
   * Update a game score
   */
  static async update(id: number, score: Partial<NewGameScore>): Promise<GameScore | null> {
    const updateFields = [];
    const values = [];
    let valueCounter = 1;
    ...
    for (const field of fields) {
      if (score[field as keyof Partial<NewGameScore>] !== undefined) {
        updateFields.push(`${field} = $${valueCounter}`);
        values.push(score[field as keyof Partial<NewGameScore>]);
        valueCounter++;
      }
    }
   ...
   */
  static async getById(id: number): Promise<GameScore | null> {
   ...
   */
  static async getGameWithScores(gameId: number): Promise<any> {
    const query = `
      SELECT 
        g.id as game_id,
        g.date_played,
